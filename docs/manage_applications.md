...
## Acceptance Criteria
1. **Happy path**: On the tutor applications page, if I approve a tutor application, I see an "Application approved" message, the pending application is no longer present on the page, and the learner is now a tutor
...
3. **Already approved**: If an approve or reject request is made for a user who is already a tutor, an error message should be shown
4. **Not real**: If an approve or reject request is made for a user who does not exist, an error message should be shown
...
### Views
- A **admin/tutor_applications/index.html.erb** that shows a list of all pending tutor applications (including approve and deny buttons).

### Controllers
- An **Admin::TutorApplicationsController** with `index`, `approve`, and `reject` actions.
